...
import 'package:sewoapp/data_bank/bloc/data_bank_ubah_bloc.dart';
import 'package:sewoapp/data_bank/data/data_bank_apidata.dart';
import 'package:sewoapp/data_bank/data_bank_tambah.dart';
import 'package:sewoapp/data_bank/data_bank_tampil.dart';
import 'package:sewoapp/data_bank/data_bank_ubah.dart';
...
class DataBankScreen extends StatefulWidget {
  static const routeName = '/data_bank';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: MultiBlocListener(
        listeners: [
          BlocListener<DataBankHapusBloc,
              DataBankHapusState>(
            listener: (context, state) {
              if (state is DataBankHapusLoadSuccess) {
                fetchData();
              }
            },
          ),
          BlocListener<DataBankUbahBloc, DataBankUbahState>(
            listener: (context, state) {
              if (state is DataBankUbahLoadSuccess) {
                fetchData();
              }
            },
          ),
          BlocListener<DataBankSimpanBloc, DataBankSimpanState>(
            listener: (context, state) {
              if (state is DataBankSimpanLoadSuccess) {
                fetchData();
              }
            },
          ),
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        children: [
                          TombolTambahWidget(
                            onPress: () {
                              Navigator.pushNamed(
                                context,
                                DataBankTambahScreen.routeName,
                                arguments: DataBankTambahArguments(
                                  data: DataBank(),
                                  judul: "Tambah Data Bank",
                                ),
                              );
                            },
                          ),
                      ...
                      Builder(builder: (context) {
                        final stateData =
                            context.watch<DataBankBloc>().state;
                        final stateHapus =
                            context.watch<DataBankHapusBloc>().state;
                        if (stateData is DataBankLoading ||
                            stateHapus is DataBankHapusLoading) {
                          return const LoadingWidget();
                        }
                        if (stateData is DataBankLoadSuccess) {
                          List<DataBankApiData> data =
                          ...
                          return ListView.builder(
                            ...
                            itemBuilder: ((context, index) {
                              return DataBankTampil(
                                data: data[index],
                                onTapHapus: (value) async {
                                  BlocProvider.of<DataBankHapusBloc>(
                                    context,
                                  ).add(
                                    FetchDataBankHapus(
                                      data: value,
                                ...
                                onTapEdit: (value) async {
                                  var data = DataBank(
									...
									fotoLogoBank : value.fotoLogoBank,
                                  );
                                  await Navigator.of(context).pushNamed(
                                    DataBankUbahScreen.routeName,
                                    arguments: DataBankUbahArguments(
                                      data: data,
